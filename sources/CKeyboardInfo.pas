...
type
  ...
  public
    ...
    // Obtener la propiedad <Password> como string
    function GetAllPasswordAsString(index:integer):string;
    // Obtener el número de elementos disponibles de la propiedad <Password>
    function GetAllPasswordCount:integer;
  ...
  published
    ...
    ///  LastErrorCode captura el último código de error informado por el dispositivo lógico.
    /// </summary>
    property Layout:string read FLayout write FLayout stored False;
    /// <summary>
    ///  LastErrorCode captura el último código de error informado por el dispositivo lógico.
    /// </summary>
    ...
    ///  LastErrorCode captura el último código de error informado por el dispositivo lógico.
    /// </summary>
    property NumberOfFunctionKeys:integer read FNumberOfFunctionKeys write FNumberOfFunctionKeys stored False;
    /// <summary>
    ///  Un entero que indica si la contraseña de nivel de hardware está habilitada en el teclado (valor=4), evitando 
    ///  entrada local. Otros valores son:  "Deshabilitado" (3), "No implementado" (5), "Otros" (1) y "Desconocido" 
    ///  (2).
    /// </summary>
    property Password:integer read FPassword write FPassword stored False;
    /// <summary>
    ///  Un entero que indica si la contraseña de nivel de hardware está habilitada en el teclado (valor=4), evitando 
    ///  entrada local. Otros valores son:  "Deshabilitado" (3), "No implementado" (5), "Otros" (1) y "Desconocido" 
    ///  (2).
  ...
  ///  de Microsoft
  /// </summary>
  TKeyboardInfo = class(TWMIBase)
  private
    FKeyboardProperties: TKeyboardProperties;
  protected
  ...
  published
    ...
    // propiedades de la Keyboard
    property KeyboardProperties:TKeyboardProperties read   FKeyboardProperties write FKeyboardProperties;
  end;
...
// Constantes para la propiedad Password
const 
  ENUM_STRING_PASSWORD_00 = 'Otros';
  ENUM_STRING_PASSWORD_01 = 'Desconocido';
  ENUM_STRING_PASSWORD_02 = 'Deshabilitado';
  ENUM_STRING_PASSWORD_03 = 'Habilitado';
  ENUM_STRING_PASSWORD_04 = 'No implementado';
...
// Limpiar las propiedades
procedure TKeyboardInfo.ClearProps;
begin
  Self.KeyboardProperties.FAvailability := 0;
  Self.KeyboardProperties.FCaption := EmptyStr;
  Self.KeyboardProperties.FConfigManagerErrorCode := 0;
  Self.KeyboardProperties.FConfigManagerUserConfig := False;
  Self.KeyboardProperties.FCreationClassName := EmptyStr;
  Self.KeyboardProperties.FDescription := EmptyStr;
  Self.KeyboardProperties.FDeviceID := EmptyStr;
  Self.KeyboardProperties.FErrorCleared := False;
  Self.KeyboardProperties.FErrorDescription := EmptyStr;
  Self.KeyboardProperties.FInstallDate := 0;
  Self.KeyboardProperties.FIsLocked := False;
  Self.KeyboardProperties.FLastErrorCode := 0;
  Self.KeyboardProperties.FLayout := EmptyStr;
  Self.KeyboardProperties.FName := EmptyStr;
  Self.KeyboardProperties.FNumberOfFunctionKeys := 0;
  Self.KeyboardProperties.FPassword := 0;
  Self.KeyboardProperties.FPNPDeviceID := EmptyStr;
  Self.KeyboardProperties.FPowerManagementCapabilitiesCount := 0;
  Self.KeyboardProperties.FPowerManagementCapabilitiesAsString := EmptyStr;
  SetLength(KeyboardProperties.FPowerManagementCapabilities, 0);
  Self.KeyboardProperties.FPowerManagementSupported := False;
  Self.KeyboardProperties.FStatus := EmptyStr;
  Self.KeyboardProperties.FStatusInfo := 0;
  Self.KeyboardProperties.FSystemCreationClassName := EmptyStr;
  Self.KeyboardProperties.FSystemName := EmptyStr;
end;

//: Constructor del componente
constructor TKeyboardInfo.Create(AOwner: TComponent);
begin
  inherited;
  Self.FKeyboardProperties := TKeyboardProperties.Create();
  Self.MSDNHelp := 'https://docs.microsoft.com/en-us/windows/win32/cimwin32prov/win32-keyboard';
end;

//: destructor del componente
destructor TKeyboardInfo.Destroy();
begin
  // liberar
  FreeAndNil(Self.FKeyboardProperties);
  inherited;
end;

//: Obtener la clase
function TKeyboardInfo.GetWMIClass(): string;
begin
  Result := 'Win32_Keyboard'
end;

//: Obtener Root
function TKeyboardInfo.GetWMIRoot(): string;
begin
  Result := '';
end;

//: Procedmiento de activación
procedure TKeyboardInfo.SetActive(const Value: Boolean);
begin
  // método heredado
  inherited;
end;
...
function TKeyboardProperties.GetPowerManagementCapabilities(index: integer):integer;
begin
...
procedure TKeyboardInfo.FillProperties(AIndex: integer);
var
  v:     variant;
  vNull: boolean;
  vp:    TKeyboardProperties;
begin
  // Llamar al heredado (importante)
  inherited;
  // Rellenar propiedades...
  vp := KeyboardProperties;
  ...
  GetWMIPropertyValue(Self, 'Layout', v, vNull);
  vp.FLayout := VariantStrValue(v, vNull);

  GetWMIPropertyValue(Self, 'Name', v, vNull);
  vp.FName := VariantStrValue(v, vNull);

  GetWMIPropertyValue(Self, 'NumberOfFunctionKeys', v, vNull);
  vp.FNumberOfFunctionKeys := VariantIntegerValue(v, vNull);

  GetWMIPropertyValue(Self, 'Password', v, vNull);
  vp.FPassword := VariantIntegerValue(v, vNull);
...
function TKeyboardProperties.GetAllAvailabilityAsString(index:integer):string;
begin
  case index of
...
// Obtener la propiedad como string
function TKeyboardProperties.GetAllConfigManagerErrorCodeAsString(index:integer):string;
begin
  case index of
...
// Obtener la propiedad como string
function TKeyboardProperties.GetAllPasswordAsString(index:integer):string;
begin
  case index of
    00: Result := ENUM_STRING_PASSWORD_00;
    01: Result := ENUM_STRING_PASSWORD_01;
    02: Result := ENUM_STRING_PASSWORD_02;
    03: Result := ENUM_STRING_PASSWORD_03;
    04: Result := ENUM_STRING_PASSWORD_04;
  else
    Result := STR_EMPTY;
  end;
end;
...
// Obtener la propiedad como string
function TKeyboardProperties.GetAllPowerManagementCapabilitiesAsString(index:integer):string;
begin
  case index of
...
// Obtener la propiedad como string
function TKeyboardProperties.GetAllStatusInfoAsString(index:integer):string;
begin
  case index of
...
function TKeyboardProperties.GetAllAvailabilityCount:integer;
begin
  Result := 21
end;

// Obtener núm. de elementos de la propiedad como string
function TKeyboardProperties.GetAllConfigManagerErrorCodeCount:integer;
begin
  Result := 32
end;

// Obtener núm. de elementos de la propiedad como string
function TKeyboardProperties.GetAllPasswordCount:integer;
begin
  Result := 5
end;

// Obtener núm. de elementos de la propiedad como string
function TKeyboardProperties.GetAllPowerManagementCapabilitiesCount:integer;
begin
  Result := 8
end;

// Obtener núm. de elementos de la propiedad como string
function TKeyboardProperties.GetAllStatusInfoCount:integer;
begin
  Result := 5
end;
