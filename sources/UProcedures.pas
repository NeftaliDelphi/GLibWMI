...
function VariantDoubleValue(v:variant; vIsNull:Boolean):Extended;
function VariantBooleanValue(v:variant; vIsNull:Boolean):Boolean;
function VariantDateTimeValue(v:variant; vIsNull:Boolean):TDateTime;
...
begin
  if (vIsNull) then begin
    ...
    Result := EncodeDate(StrToInt(Copy(v, 1, 4)),StrToInt(Copy(v, 5, 2)),StrToInt(Copy(v, 7, 2)));
  end;
...
begin
  ...
  try
    ...
    if (AInstanceProp = STR_EMPTY) or (AInstanceValue = STR_EMPTY) then begin
      // SQL de consulta
