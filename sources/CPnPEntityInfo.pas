...
and Play se muestran como entradas en el Administrador de dispositivos que se encuentra en el Panel de 
control.
...
type
  ...
  TPnPEntityProperties = class(TPersistent)
  private
    {%PrivateClassVars PropertiesClass%}
    FAvailability:integer;
    FCaption:string;
    FClassGuid:string;
    FCompatibleID:TArrString;
    FCompatibleIDCount:integer;
    FCompatibleIDAsString:string;    FConfigManagerErrorCode:integer;
    ...
    FHardwareID:TArrString;
    FHardwareIDCount:integer;
    FHardwareIDAsString:string;    FInstallDate:TDatetime;
  ...
  private
    ...
    function GetCompatibleID(index: integer):string;
    function GetHardwareID(index: integer):string;
  ...
  public
    ...
    property CompatibleID[index:integer]:string read GetCompatibleID;
    property CompatibleIDCount:integer read FCompatibleIDCount stored False;
    property HardwareID[index:integer]:string read GetHardwareID;
    property HardwareIDCount:integer read FHardwareIDCount stored False;
  ...
  published
    ...
    /// <summary>
    ///  La propiedad ClassGuid contiene el identificador único global (GUID) que identifica a este dispositivo 
    ///  Plug and Play de forma exclusiva.
    /// </summary>
    property ClassGuid:string read FClassGuid write FClassGuid stored False;
    /// <summary>
    ///  Lista de id. compatibles para el dispositivo. Si el programa de instalación no puede localizar un archivo 
    ///  INF que coincida con uno de los id. de hardware de un dispositivo, usa id. compatibles para localizar 
    ///  un archivo INF.
    /// </summary>
    property CompatibleIDAsString:string read FCompatibleIDAsString write FCompatibleIDAsString stored False;
    /// <summary>
    ...
    /// <summary>
    ///  La propiedad DeviceID contiene una cadena que identifica al dispositivo Plug and Play.
    /// </summary>
    ...
    /// <summary>
    ///  Lista de id. de hardware asociados con el dispositivo. Los id. de hardware son cadenas de identificación 
    ///  definidas por el proveedor que usa el programa de instalación para encontrar el dispositivo que corresponde 
    ///  a cada archivo INF.
    /// </summary>
    property HardwareIDAsString:string read FHardwareIDAsString write FHardwareIDAsString stored False;
    /// <summary>
    ///  Lista de id. de hardware asociados con el dispositivo. Los id. de hardware son cadenas de identificación 
    ///  definidas por el proveedor que usa el programa de instalación para encontrar el dispositivo que corresponde 
    ///  a cada archivo INF.
    ...
    /// <summary>
    ///  La propiedad Manufacturer contiene el nombre del fabricante del dispositivo Plug and Play.
    ///  Ejemplo: 
    ///  Acme
    /// </summary>
    ...
    /// <summary>
    ///  La propiedad PNPClass contiene el nombre del tipo de este dispositivo Plug and Play.
    /// </summary>
    property PNPClass:string read FPNPClass write FPNPClass stored False;
    /// <summary>
    ///  La propiedad PNPClass contiene el nombre del tipo de este dispositivo Plug and Play.
    /// </summary>
    ...
    /// <summary>
    ///  La propiedad Present indica si este dispositivo Plug and Play está disponible actualmente en el sist
    ///  ema.
    /// </summary>
    property Present:boolean read FPresent write FPresent stored False;
    /// <summary>
    ///  La propiedad Service contiene el nombre del servicio compatible con este dispositivo Plug and Play. Consulte 
    ///  también la clase Win32_SystemDriverPNPEntity.
    ///  Ejemplo: atapi
    /// </summary>
    property Service:string read FService write FService stored False;
    /// <summary>
    ///  La propiedad Service contiene el nombre del servicio compatible con este dispositivo Plug and Play. Consulte 
    ///  también la clase Win32_SystemDriverPNPEntity.
    ///  Ejemplo: atapi
  ...
  ///  control.
  /// </summary>
  TPnPEntityInfo = class(TWMIBase)
  private
    FPnPEntityProperties: TPnPEntityProperties;
  protected
  ...
  published
    ...
    // propiedades de la PnPEntity
    property PnPEntityProperties:TPnPEntityProperties read   FPnPEntityProperties write FPnPEntityProperties;
  end;
...
// Limpiar las propiedades
procedure TPnPEntityInfo.ClearProps;
begin
  Self.PnPEntityProperties.FAvailability := 0;
  Self.PnPEntityProperties.FCaption := EmptyStr;
  Self.PnPEntityProperties.FClassGuid := EmptyStr;
  Self.PnPEntityProperties.FCompatibleIDCount := 0;
  Self.PnPEntityProperties.FCompatibleIDAsString := EmptyStr;
  SetLength(PnPEntityProperties.FCompatibleID, 0);
  Self.PnPEntityProperties.FConfigManagerErrorCode := 0;
  Self.PnPEntityProperties.FConfigManagerUserConfig := False;
  Self.PnPEntityProperties.FCreationClassName := EmptyStr;
  Self.PnPEntityProperties.FDescription := EmptyStr;
  Self.PnPEntityProperties.FDeviceID := EmptyStr;
  Self.PnPEntityProperties.FErrorCleared := False;
  Self.PnPEntityProperties.FErrorDescription := EmptyStr;
  Self.PnPEntityProperties.FHardwareIDCount := 0;
  Self.PnPEntityProperties.FHardwareIDAsString := EmptyStr;
  SetLength(PnPEntityProperties.FHardwareID, 0);
  Self.PnPEntityProperties.FInstallDate := 0;
  Self.PnPEntityProperties.FLastErrorCode := 0;
  Self.PnPEntityProperties.FManufacturer := EmptyStr;
  Self.PnPEntityProperties.FName := EmptyStr;
  Self.PnPEntityProperties.FPNPClass := EmptyStr;
  Self.PnPEntityProperties.FPNPDeviceID := EmptyStr;
  Self.PnPEntityProperties.FPowerManagementCapabilitiesCount := 0;
  Self.PnPEntityProperties.FPowerManagementCapabilitiesAsString := EmptyStr;
  SetLength(PnPEntityProperties.FPowerManagementCapabilities, 0);
  Self.PnPEntityProperties.FPowerManagementSupported := False;
  Self.PnPEntityProperties.FPresent := False;
  Self.PnPEntityProperties.FService := EmptyStr;
  Self.PnPEntityProperties.FStatus := EmptyStr;
  Self.PnPEntityProperties.FStatusInfo := 0;
  Self.PnPEntityProperties.FSystemCreationClassName := EmptyStr;
  Self.PnPEntityProperties.FSystemName := EmptyStr;
end;

//: Constructor del componente
constructor TPnPEntityInfo.Create(AOwner: TComponent);
begin
  inherited;
  Self.FPnPEntityProperties := TPnPEntityProperties.Create();
  Self.MSDNHelp := 'https://docs.microsoft.com/en-us/windows/win32/cimwin32prov/win32-pnpentity';
end;

//: destructor del componente
destructor TPnPEntityInfo.Destroy();
begin
  // liberar
  FreeAndNil(Self.FPnPEntityProperties);
  inherited;
end;

//: Obtener la clase
function TPnPEntityInfo.GetWMIClass(): string;
begin
  Result := 'Win32_PnPEntity'
end;

//: Obtener Root
function TPnPEntityInfo.GetWMIRoot(): string;
begin
  Result := '';
end;

//: Procedmiento de activación
procedure TPnPEntityInfo.SetActive(const Value: Boolean);
begin
  // método heredado
  inherited;
end;

// Acceso a los elementos de la propiedad <CompatibleID>
function TPnPEntityProperties.GetCompatibleID(index: integer):string;
begin
  if (index >= Self.FCompatibleIDCount) then begin
    Index := Self.FCompatibleIDCount - 1;
  end;
  Result := Self.FCompatibleID[index];
end;

// Acceso a los elementos de la propiedad <HardwareID>
function TPnPEntityProperties.GetHardwareID(index: integer):string;
begin
  if (index >= Self.FHardwareIDCount) then begin
    Index := Self.FHardwareIDCount - 1;
  end;
  Result := Self.FHardwareID[index];
end;

// Acceso a los elementos de la propiedad <PowerManagementCapabilities>
function TPnPEntityProperties.GetPowerManagementCapabilities(index: integer):integer;
begin
...
procedure TPnPEntityInfo.FillProperties(AIndex: integer);
var
  v:     variant;
  vNull: boolean;
  vp:    TPnPEntityProperties;
begin
  // Llamar al heredado (importante)
  inherited;
  // Rellenar propiedades...
  vp := PnPEntityProperties;
  ...
  GetWMIPropertyValue(Self, 'ClassGuid', v, vNull);
  vp.FClassGuid := VariantStrValue(v, vNull);

  GetWMIPropertyValue(Self, 'CompatibleID', v, vNull);
  vp.FCompatibleIDAsString := VariantStrValue(v, vNull);
  StringToArrayString(vp.FCompatibleIDAsString, vp.FCompatibleID);
  vp.FCompatibleIDCount := Length(vp.FCompatibleID);
  ...
  GetWMIPropertyValue(Self, 'HardwareID', v, vNull);
  vp.FHardwareIDAsString := VariantStrValue(v, vNull);
  StringToArrayString(vp.FHardwareIDAsString, vp.FHardwareID);
  vp.FHardwareIDCount := Length(vp.FHardwareID);
  ...
  GetWMIPropertyValue(Self, 'PNPClass', v, vNull);
  vp.FPNPClass := VariantStrValue(v, vNull);
  ...
  GetWMIPropertyValue(Self, 'Present', v, vNull);
  vp.FPresent := VariantBooleanValue(v, vNull);

  GetWMIPropertyValue(Self, 'Service', v, vNull);
  vp.FService := VariantStrValue(v, vNull);
...
function TPnPEntityProperties.GetAllAvailabilityAsString(index:integer):string;
begin
  case index of
...
// Obtener la propiedad como string
function TPnPEntityProperties.GetAllConfigManagerErrorCodeAsString(index:integer):string;
begin
  case index of
...
// Obtener la propiedad como string
function TPnPEntityProperties.GetAllPowerManagementCapabilitiesAsString(index:integer):string;
begin
  case index of
...
// Obtener la propiedad como string
function TPnPEntityProperties.GetAllStatusInfoAsString(index:integer):string;
begin
  case index of
...
function TPnPEntityProperties.GetAllAvailabilityCount:integer;
begin
  Result := 21
end;

// Obtener núm. de elementos de la propiedad como string
function TPnPEntityProperties.GetAllConfigManagerErrorCodeCount:integer;
begin
  Result := 32
end;

// Obtener núm. de elementos de la propiedad como string
function TPnPEntityProperties.GetAllPowerManagementCapabilitiesCount:integer;
begin
  Result := 8
end;

// Obtener núm. de elementos de la propiedad como string
function TPnPEntityProperties.GetAllStatusInfoCount:integer;
begin
  Result := 5
end;
